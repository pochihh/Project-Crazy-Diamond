...
- We are using SPI DMA on the 28379D right now, which we have a proven working DMA example in under /DSP_28379D
...
- One DSP should support: 
    - 6 motors encoder readings (use 2 of EQE hardware encoder reader, 4 GPIO interrupt. I believe there are 3 EQE for one DSP, but only 2 are wired out for the EVM. Correct me if I am wrong)
    - 4 additional optional encoder readings, for slower applications, like linear encoder for motion detection
    - 4/8 ADC readings (single/differential modes)
    - SPI master to control other devices, for example a stepper motor driver, TMC5160
...
- CPU/CLA:
    - CPU1: main code, ADC measure,  SPI master in the future
    - CPU2: encoder interrupt for encoder counting only
    - CLA1: real-time control algorithm accerlarator for the 6 motors. Put a LQR/PID for now
    - CLA2: unused
...
- Include both Flash and RAM .cmd
