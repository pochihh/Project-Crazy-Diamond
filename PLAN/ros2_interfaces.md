...
/ui_bridge ──► /robot ── public units ──► ros2_control /bridge
    │                                         │
    └──── services/status ─► /bridge_manager  ├─ 1 kHz SPI ─► DSP 0
                                               └─ mock SPI ─► DSP 1…n
```

| Node/process | Package | Responsibility |
|---|---|---|
| `/bridge` | `bridge` | Renamed `ros2_control_node`; owns controller manager, hardware components, and the 1 kHz real-time loop |
...
| `/dsp/<id>/axis_state` | `interfaces/msg/AxisStateArray` | `/bridge_manager` → `/ui_bridge`, tools | 50 Hz; best-effort, keep last 1 | UI-ready position, velocity, duty, current, mode, enable, and per-axis fault state |
...
The ros2_control update loop remains 1 kHz internally. Publishing 1 kHz DDS
telemetry is intentionally omitted; recordings receive full-rate samples from
the in-process real-time ring instead.

## 4. Application message contracts

All arrays are bounded to six axes and must have matching lengths. Receivers
reject unknown `dsp_id`, duplicate joint names, non-finite values, and invalid
modes.
...
- firmware/protocol versions and axis count;
...
| `/bridge_manager/apply_configuration` | `interfaces/srv/ApplyConfiguration` | `/bridge_manager` | Validate a structured config, apply raw limits/gains while disarmed, await the DSP revision, then atomically persist YAML |
...
| `/dsp/<id>/arm` | `std_srvs/srv/Trigger` | `/bridge_manager` | Arm only after config revision, limits, faults, and external-enable checks pass |
...
| `/dsp/<id>/zero_encoders` | `interfaces/srv/AxisMask` | `/bridge_manager` | Zero selected encoder counts while disarmed |
...
by the web UI. There are no project actions in the first pass.

## 6. Real-time boundary

The CPU 3 controller-manager update thread:
...
- performs one fixed 128-byte full-duplex SPI transaction per active physical
  DSP per cycle;
...
and PID commands, commits the complete revision, checks acknowledgements, and
confirms the active revision before allowing arm. A failed apply leaves the
prior DSP revision and YAML file
active. A successful UI edit is written via temporary file, `fsync`, and atomic
rename.

Hardware pin routing and PWM slew are reviewed board-profile data, not free-form
web edits. Encoder cycles, quadrature multiplier, gear ratio, sign, zero, public
units, limits, and controller gains are editable while disarmed.
...
until an analysis consumer needs it.
...
manual commands, arm/disarm/clear/zero, plots, recording, download, and diagnostic
views. A browser disconnect is not a safety mechanism; the DSP watchdog is.
...
docker/
dsp/
  f28379d/                 # current DSP_28379D contents
ros2_ws/
  src/
