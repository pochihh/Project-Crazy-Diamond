...
//     - XINT1 on GPIO123 (CS mirror): both edges, guarded by g_cs_active
//     - Double-buffered TX and RX; CPU calls SpiDma_updateTx() from main loop
//
//   CS edge debounce:
//     g_cs_active prevents processing the same frame twice if the XINT1
...
static void configCsInterrupt(void)
{
    ...
    Interrupt_register(INT_XINT1, &SpiDma_csIsr);
