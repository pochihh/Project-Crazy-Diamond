...
// FILE:   main.c
//
// TITLE:  Integration main — DMA SPI + CLA PID + Encoders + ADC + Motors
//
// DESCRIPTION:
//   Full robot control loop:
//     - SPI DMA slave: receives RPi commands (ref[6], cmd)
//     - CLA PID:       runs at 1 kHz, computes u[6] from ref/pos
//     - Encoders:      EQEP1/2/3 (CPU1) + GPIO quadrature via CPU2 IPC
//     - ADC:           4 channels (ADCA-D SOC0) sampled each main loop
//     - Motors:        6-axis H-bridge driven from CLA output
//
//   Control loop flow:
//     1. main loop polls encoders and ADC every ~1 ms
//     2. CPU writes gEncPos[] → gCpuToCla.pos[] before each CLA tick
//     3. CLA Task 1 fires at 1 kHz:  reads pos/ref, outputs u[]
...
//
// BUILD:
//   Add to both RAM and FLASH configs in .cproject:
//     spi_dma.c, cla_setup.c, cla_pid.cla, encoders.c, motor.c, main.c
//   Source files: device/device.c
//   Linker cmd: 2837xD_RAM_lnk_cpu1.cmd (RAM) or 2837xD_FLASH_lnk_cpu1.cmd (FLASH)
//
//#############################################################################
...
//
// ============ ADC (4 channels: ADCA-D SOC0) ============
//
static uint16_t gAdc[6] = {0};   // indices 0-3 valid; 4-5 unused (set to 0)
...
static void ADC_setup(void)
{
    ...
    ADC_setupSOC(ADCA_BASE, ADC_SOC_NUMBER0, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN3, 15U);
    ADC_setupSOC(ADCB_BASE, ADC_SOC_NUMBER0, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN3, 15U);
    ADC_setupSOC(ADCC_BASE, ADC_SOC_NUMBER0, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN3, 15U);
    ADC_setupSOC(ADCD_BASE, ADC_SOC_NUMBER0, ADC_TRIGGER_SW_ONLY, ADC_CH_ADCIN3, 15U);
...
static void ADC_sampleTick(void)
{
    ...
    gAdc[0] = ADC_readResult(ADCARESULT_BASE, ADC_SOC_NUMBER0);
    gAdc[1] = ADC_readResult(ADCBRESULT_BASE, ADC_SOC_NUMBER0);
    gAdc[2] = ADC_readResult(ADCCRESULT_BASE, ADC_SOC_NUMBER0);
    gAdc[3] = ADC_readResult(ADCDRESULT_BASE, ADC_SOC_NUMBER0);
    gAdc[4] = 0U;
    gAdc[5] = 0U;
}
...
//
// Runs from PIE 11.1 ISR after each 1 kHz CLA tick.
